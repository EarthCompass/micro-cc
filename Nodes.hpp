...
public:
  std::string name;
  bool isType;
  IndentifierExpr(std::string &name,bool isType) : name(name),isType(isType) {}
};
...
public:
  ...
  CallExpr(std::unique_ptr<IndentifierExpr> &callee, std::vector<std::unique_ptr<Expr>> args)
      : callee(std::move(callee)), args(std::move(args)) {}
};
class CoumpondStmt: public Stmt{
  public:
  std::vector<std::unique_ptr<Stmt>> stmts;
  CoumpondStmt(std::vector<std::unique_ptr<Stmt>> &stmts): stmts(std::move(stmts)){}
};
